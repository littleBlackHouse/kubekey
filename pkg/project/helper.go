...
func dealVarsFiles(p *kkprojectv1.Play, baseFS fs.FS, pbPath string) error {
	for _, file := range p.VarsFiles {
		...
		if err := yaml.Unmarshal(data, &node); err != nil {
			return fmt.Errorf("unmarshal yaml file %q failed: %w", filepath.Join(filepath.Dir(pbPath), file), err)
		}
		...
		if err != nil {
			return fmt.Errorf("combine maps file %q failed: %w", filepath.Join(filepath.Dir(pbPath), file), err)
		}
	}
...
func convertRoleVars(baseFS fs.FS, roleBase string, roleVars map[string]any) (map[string]any, error) {
	...
	if mainDefault != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("read defaults variable file %q failed: %w", mainDefault, err)
		}

		var vars map[string]any
		var node yaml.Node // marshal file on defined order
		if err := yaml.Unmarshal(mainData, &node); err != nil {
			return nil, fmt.Errorf("unmarshal defaults variable yaml file %q failed: %w", mainDefault, err)
		}
		if err := node.Decode(&vars); err != nil {
			return nil, fmt.Errorf("decode defaults variable yaml file %q failed: %w", mainDefault, err)
		}
